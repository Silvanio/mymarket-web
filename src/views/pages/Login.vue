<template>
  <div>
      ...
      <div class="p-grid p-fluid p-justify-center margin-phone">
        ...
        <div class="p-md-5 p-nogutter  p-shadow-5  div-right">
          ...
          <div class="p-grid p-fluid p-justify-center ">
            ...
            <div class="p-col-6">
              <Button :label="$t('login.lbl_create')" class="p-button-link forgot-password" @click="openCreateAccount"/>
            </div>
            ...
            <div class="p-col-11">
              <Password id="password" v-model="userModel.password" :feedback="false"
                        :placeholder="$t('login.lbl_password')"
                        :class="{'p-invalid':$v.userModel.password.$invalid && submitted}"/>
              <small v-show="$v.userModel.password.$invalid && submitted"
                     class="p-error">{{ $t('msg_required') }}</small>
            </div>
            ...
            <div class="p-col-5">
              <Button :label="$t('login.msg_forgot_password')" class="p-button-link forgot-password"
    ...
    <Dialog :header="$t('login.msg_forgot_password')" :visible.sync="displayMaximizable" :maximizable="true"
            :modal="true">
    ...
    <Dialog :header="$t('login.lbl_create')" :visible.sync="displayCreateAccount" :maximizable="true" :modal="true">
      <div class="p-grid p-fluid">
        <div class="p-col-12"></div>

        <div class="p-col-12">
          <InputText id="createName" v-model="userCreateModel.name" type="text" :placeholder="$t('account.lbl_name')"/>
        </div>
        <div class="p-col-12">
          <InputText id="createUserName" v-model="userCreateModel.username" type="text"
                     :placeholder="$t('account.lbl_username')"   />
        </div>
        <div class="p-col-12">
          <InputText id="createEmail" v-model="userCreateModel.email" type="text"
                     :placeholder="$t('account.lbl_email')"/>
        </div>
        <div class="p-col-12">
          <InputText id="createPasseword" v-model="userCreateModel.password" type="password"
                     :placeholder="$t('account.lbl_password')"/>
        </div>
        <div class="p-col-12">
          <InputText id="createNewPasseword" v-model="userCreateModel.confirmPassword" type="password"
                     :placeholder="$t('account.lbl_confirm_pass')"/>
        </div>
      </div>
      <template #footer>
        <Button :label="$t('account.lbl_create_account')" icon="pi pi-check" @click="createAccount();"/>
        <Button :label="$t('lbl_close')" icon="pi pi-times" @click="closeCreateAccount" class="p-button-secondary"/>
      </template>
    </Dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    createAccount() {
      if (!this.userCreateModel.password || !this.userCreateModel.confirmPassword) {
      ...
      loginService.createUserMyMarket(this.userCreateModel).then((response) => {
